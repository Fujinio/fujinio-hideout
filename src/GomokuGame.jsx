...
const OPENINGS = {
  '0,1': { name: '直珠', description: '最も基本的な布石。バランスの取れた展開になりやすい。' },
  '0,-1': { name: '直珠', description: '最も基本的な布石。バランスの取れた展開になりやすい。' },
  '1,0': { name: '花月', description: '攻撃的な布石。黒が有利とされる。' },
  '-1,0': { name: '花月', description: '攻撃的な布石。黒が有利とされる。' },
  '1,1': { name: '斜月', description: '変化に富んだ展開になる布石。' },
  '-1,-1': { name: '斜月', description: '変化に富んだ展開になる布石。' },
  '1,-1': { name: '残月', description: '守備的な布石。白が対応しやすい。' },
  '-1,1': { name: '残月', description: '守備的な布石。白が対応しやすい。' },
  '0,2': { name: '雨月', description: '変則的な布石。相手を惑わせる。' },
  '0,-2': { name: '雨月', description: '変則的な布石。相手を惑わせる。' },
  '2,0': { name: '金星', description: '積極的な攻めの布石。' },
  '-2,0': { name: '金星', description: '積極的な攻めの布石。' },
  '2,1': { name: '松月', description: '攻守のバランスが良い布石。' },
  '-2,-1': { name: '松月', description: '攻守のバランスが良い布石。' },
  '1,2': { name: '丘月', description: '柔軟な対応が可能な布石。' },
  '-1,-2': { name: '丘月', description: '柔軟な対応が可能な布石。' },
  '2,2': { name: '新月', description: '珍しい布石。変則的な展開に。' },
  '-2,-2': { name: '新月', description: '珍しい布石。変則的な展開に。' },
  '2,-1': { name: '瑞星', description: '攻撃重視の布石。黒有利。' },
  '-2,1': { name: '瑞星', description: '攻撃重視の布石。黒有利。' },
  '1,-2': { name: '山月', description: '守りを重視した布石。' },
  '-1,2': { name: '山月', description: '守りを重視した布石。' },
  '2,-2': { name: '嵐月', description: '激しい攻防が予想される布石。' },
  '-2,2': { name: '嵐月', description: '激しい攻防が予想される布石。' },
};
...
export default function GomokuGame() {
  ...
  const isForbiddenMove = (board, row, col) => {
    ...
    for (let i = 0; i < directions.length; i++) {
      ...
      if (checkOpenThree(board, row, col, dx, dy)) {
        if (i === 0) openThrees.horizontal = true;
        if (i === 1) openThrees.vertical = true;
        if (i === 2) openThrees.diagonal1 = true;
        if (i === 3) openThrees.diagonal2 = true;
      }
    ...
    const openThreeCount = Object.values(openThrees).filter(Boolean).length;
    
    if (openThreeCount >= 2) return '三三（両端が空いた三を同時に2つ作る禁じ手）';
    if (openFourCount >= 2) return '四四（両端のどちらかが空いた四を同時に2つ作る禁じ手）';
    if (maxLength > 5) return '長連（6個以上連続で並べる禁じ手）';
  ...
  const getAIMove = (board, difficulty, aiColor, lastPlayerMove) => {
    ...
    for (let i = 0; i < BOARD_SIZE; i++) {
      for (let j = 0; j < BOARD_SIZE; j++) {
        ...
        // 連珠モードでの布石制限を考慮
        if (ruleMode === 'renju') {
          const moveCount = totalMoves;
          
          if (moveCount === 0) {
            if (i !== center || j !== center) continue;
          } else if (moveCount === 1) {
            const rowDiff = Math.abs(i - center);
            const colDiff = Math.abs(j - center);
            if (rowDiff > 1 || colDiff > 1 || (rowDiff === 0 && colDiff === 0)) continue;
          } else if (moveCount === 2) {
            const rowDiff = Math.abs(i - center);
            const colDiff = Math.abs(j - center);
    ...
    if (difficulty >= 3) {
      const moves = candidates.map(([row, col]) => {
        ...
        // 勝ち筋があれば最優先
        if (myScore >= 90000) {
          return { row, col, score: 999999 };
        }
        
        // 難易度3: バランス型（攻守ほぼ均等）
        if (difficulty === 3) {
          score = myScore * 1.2 + defScore * 1.2 + Math.floor(Math.random() * 100);
        }
        
        // 難易度4: 防御やや優先、でも攻めもしてくる
        if (difficulty === 4) {
          score = myScore * 1.5 + defScore * 1.8 + Math.floor(Math.random() * 200);
        }
        
        // 難易度5: 勝ち筋重視、積極的に攻める
        if (difficulty === 5) {
          score = myScore * 2.0 + defScore * 1.5 + Math.floor(Math.random() * 300);
        }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full">
          <h1 className="text-4xl font-bold text-center mb-2 text-gray-800">五目並べ</h1>
          <p className="text-center text-gray-600 mb-8">ゲームモードを選択</p>
  ...
  if (gameMode === 'ai' && !ruleMode) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full">
          <h1 className="text-3xl font-bold text-center mb-2 text-gray-800">ルールを選択</h1>
          <p className="text-center text-gray-600 mb-8">対戦ルールを選んでください</p>
          ...
          <div className="space-y-4">
            ...
            >
              五目並べ
              <div className="text-sm font-normal mt-1">通常ルール</div>
            </button>
            ...
            >
              連珠
              <div className="text-sm font-normal mt-1">黒に禁じ手あり</div>
            </button>
          </div>
  ...
  if (gameMode === 'ai' && !playerColor) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full">
          <h1 className="text-3xl font-bold text-center mb-2 text-gray-800">先手・後手を選択</h1>
          <p className="text-center text-gray-600 mb-8">あなたの色を選んでください</p>
          ...
          <div className="space-y-4">
            ...
            >
              黒（先手）
              <div className="text-sm font-normal mt-1">最初に打つ</div>
            </button>
            ...
            >
              白（後手）
              <div className="text-sm font-normal mt-1">後から打つ</div>
            </button>
  ...
  if (gameMode === 'ai' && !difficulty) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full">
          <h1 className="text-3xl font-bold text-center mb-2 text-gray-800">難易度を選択</h1>
          <p className="text-center text-gray-600 mb-8">AIの強さを選んでください</p>
          ...
          <div className="space-y-3">
            ...
            >
              ⭐ やさしい
              <div className="text-sm font-normal mt-1">初心者向け・周囲3マス</div>
            </button>
            ...
            >
              ⭐⭐ ふつう
              <div className="text-sm font-normal mt-1">バランス重視</div>
            </button>
            ...
            >
              ⭐⭐⭐ むずかしい
              <div className="text-sm font-normal mt-1">攻守を考える</div>
            </button>
            ...
            >
              ⭐⭐⭐⭐ 超むずかしい
              <div className="text-sm font-normal mt-1">最善手を狙う</div>
            </button>
            ...
            >
              ⭐⭐⭐⭐⭐ 達人
              <div className="text-sm font-normal mt-1">禁じ手ハメあり</div>
            </button>
          </div>
  ...
  const difficultyName = ['', 'やさしい', 'ふつう', 'むずかしい', '超むずかしい', '達人'][difficulty] || '';
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-3 sm:p-6 w-full max-w-2xl">
        <div className="flex justify-between items-center mb-3 sm:mb-4 flex-wrap gap-2">
          ...
          <div className="flex gap-2 flex-wrap">
            <button
              ...
              title={isSoundOn ? '効果音 ON' : '効果音 OFF'}
            >
              <span>{isSoundOn ? '🔊' : '🔇'}</span>
            </button>
        ...
        {ruleMode === 'renju' && (
          <div className="mb-2 p-2 bg-cyan-50 border border-cyan-200 rounded-lg text-xs sm:text-sm text-cyan-800 flex items-center gap-2">
            <AlertCircle size={16} />
            <span>連珠モード: 黒の禁じ手（三三・四四・長連）あり</span>
          </div>
        )}
          ...
          <div className="mb-3 sm:mb-4 text-center">
            <div className="inline-flex items-center gap-2 bg-gray-100 px-4 sm:px-6 py-2 rounded-full">
              ...
              <span className="font-bold text-sm sm:text-base">
                {isAIThinking ? 'AIが考え中...' : `${currentPlayer === 'black' ? '黒' : '白'}の番 ${currentPlayer === 'black' ? '(先手)' : '(後手)'}`}
              </span>
            </div>
          </div>
        )}
              ...
              row.map((cell, colIndex) => (
                <div
                  ...
                  className={`absolute cursor-pointer flex items-center justify-center transition-all ${
                    !isPlaceableCell(rowIndex, colIndex) && !board[rowIndex][colIndex] ? 'opacity-30 bg-black' : ''
                  } ${
                    selectedCell && selectedCell[0] === rowIndex && selectedCell[1] === colIndex ? 'ring-2 ring-red-500' : ''
                  }`}
                  style={{
                ...
                >
                  {selectedCell && selectedCell[0] === rowIndex && selectedCell[1] === colIndex && !board[rowIndex][colIndex] && (
                    <div className="absolute w-full h-full flex items-center justify-center z-10">
                      <div className="w-4 h-4 sm:w-6 sm:h-6 bg-red-500 rounded-full animate-pulse"></div>
                    </div>
                  )}
